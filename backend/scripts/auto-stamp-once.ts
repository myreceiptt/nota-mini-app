...
async function main() {
  ...
  const index = await loadProgress();
  ...
  const contractRead = await ethers.getContractAt("MyReceiptStamp", receipt);
  ...
  const contract = await ethers.getContractAt("MyReceiptStamp", receipt, sender);
